...
export const addReport=async(req,res,next) =>{
    ...
    //prepare data
     ...
     if(!createdreport){
        return next( new AppError (message.report.fileToCreate,500))
     }
...
export const updateReport= async (req,res,next) => {
        ...
        if(!reportExist){
            return next( new AppError (message.report.notFound,404))
        }
        //check existance
     const studentExist=await Student.findById(studentId)
     if(!studentExist){
        ...
        if(!updatereport){
            return next( new AppError (message.report.fileToUpdate,500))
        }
        //send response
...
export const getspecificReport= async (req,res,next) => {
    ...
    report?
    res.status(200).json({message:"get successfully", success:true,data:report})
        : next (new AppError (message.report.notFound,404))
}
//-------------5-delete Report-------------------------------------
